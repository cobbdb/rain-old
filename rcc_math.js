...
function getRainfall(mo_index) {
    var str_val = stringToInt(
        $("#io_rain_inputs_" + mo_index + "_box").val()
    );
...
function getUsage(mo_index) {
    var str_val = stringToInt(
        $("#input_usage_" + mo_index + "_box").val()
    );
...
function getDistance() {
    var i_dist = stringToInt(
        $("#input_storage_box").val()
    );
...
function setAnnualHarvest() {
    ...
    $("#out_harvest").html(
        harvest + ' ' + unit_name_L[current_unit]
    );
}
...
function setTankWeight() {
    ...
    $("#out_weight").html(
        weight + ' ' + unit_name_kg[current_unit]
    );
}
...
function calc_supply(mo_index, full_tank) {
    ...
    if (mo_index == 0) {
        // jan
        stored = (full_tank) ? current_tank_size : 0;
    } else {
...
function calc_storage_all(full_tank) {
    var tank_min = 0,
        tank_max = storage_max, // bounds in L
        tank_size = tank_min, // starting point
        good_size,
        i,
        exit = false,
        /**
