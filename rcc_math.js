...
function getTankSize()
{
	...
	if( tank_size == -99 ) // too large
	{
		...
		else // max_level == storage_max
			tank_size = "4000000";
	}
	else if( tank_size == -88 ) // too small
		tank_size = "10";
...
function setFirstFlush()
{
	...
	$("#out_first_flush").html( gal_low + " - " +
								gal_high + " " + unit_name_L[current_unit] );
}
...
function calc_storage_all( full_tank )
{
	...
	if( err_code > 0 ) // too large
		tank_size = -99;
	else if( err_code < 0 ) // too small
		tank_size = -88;
